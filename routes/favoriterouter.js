      ...
      .then(
        (favorites) => {
          if (favorites.length) {
            ...
            const existingDishIds = favorite.dishes.map((object) =>
              object._id.toString()
            );
            const newDishesToAdd = dishesToAdd.filter(
...
favoriteRouter
  ...
  .post(cors.corsWithOptions, authenticate.verifyUser, addToFavorites)
...
favoriteRouter
  ...
  .get(cors.corsWithOptions, authenticate.verifyUser, (_req, res, _next) => {
    Favorites.findOne({ user: req.user._id })
      .then(
        (favorites) => {
          const exists =
            favorites && favorites.dishes.indexOf(req.params.dishId) >= 0;
          res.statusCode = 200;
          res.setHeader("Content-Type", "application/json");
          return res.json({
            exists: exists,
        ...
        (favorites) => {
          if (favorites.length) {
            ...
            if (existingDishIds.includes(req.params.dishId)) {
              favorite.dishes = existingDishIds.filter(
